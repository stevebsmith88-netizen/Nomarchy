...
export async function getProfile(userId) {
  const { data, error } = await supabase
    .from("profiles")
    .select("id, username, display_name, city, is_owner, is_public, notifications_seen_at")
    .eq("id", userId)
    .single();
  if (error) throw error;
  return data;
...
// Two things worth a bell icon for: someone new following you, and anyone
// you follow crowning (or re-crowning, via a coup) a restaurant. Computed
// on read from data that already exists - no separate notifications table,
// nothing to keep in sync. A private Court member's crowns are correctly
// invisible here too, for free: thrones RLS already hides them from
// everyone but the owner, same as everywhere else in the app.
export async function loadNotifications(userId, since) {
...
// instead, but only surfaces people who've kept their profile public -
// the same is_public toggle that already governs their kingdom link and
// Court visibility applies here too, so going private also means not
// being listed for strangers to find.
